#!/usr/bin/env python3
"""
Pixelmon-like Pygame MVP (v4.1 — Zoom)
- True zoom: world renders to a smaller viewport surface and scales up to screen.
- Toggleable minimap (M), camera/viewport scrolling, --level N param, optional --zoom Z (default 2.0).
- Bigger on-screen sprites/tiles without changing level files.
"""
...
def parse_args():
    ...
    p.add_argument("--zoom", type=float, default=2.0)  # 2x zoom by default
    try:
...
pygame.display.set_caption("Pixelmon Pygame MVP v4.1 (Zoom)")
...
class Camera:
    def __init__(self, map_w_tiles, map_h_tiles):
        ...
        self.x = 0; self.y = 0
    def center_on(self, rect, viewport_w, viewport_h):
        self.x = rect.centerx - viewport_w//2
        self.y = rect.centery - viewport_h//2
        self.x = max(0, min(self.x, self.w_px - viewport_w))
        self.y = max(0, min(self.y, self.h_px - viewport_h))
    def apply(self, pos):
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.apricorns = 0
        self.balls = 3
        self.team = []; self.caught_species = set()
...
class LevelWorld:
    ...
    def __init__(self, grid, w_tiles, h_tiles):
        ...
        self.bushes = []; self.mons = pygame.sprite.Group()
        self._respawn_t=0.0; self._bush_t=0.0
        self._init_bushes(); self._init_mons()
    ...
    def draw(self, surf, cam, viewport_w, viewport_h):
        first_tx = cam.x // TILE; first_ty = cam.y // TILE
        tiles_x = viewport_w//TILE + 2; tiles_y = viewport_h//TILE + 2
...
class Battle:
    def __init__(self, player: "Player", wild: Mon):
        self.player = player; self.wild = wild; self.active = True
        self.message = f"A wild {wild.name} (Lv{wild.level}) appeared!"; self.cooldown = 0.0
    ...
    def _hp_box(self, surf, pos, label, hp, maxhp):
        pygame.draw.rect(surf, WHITE, (*pos, 320, 48), 2); draw_text(surf, label, (pos[0]+10, pos[1]+6))
        ratio = max(0, hp)/maxhp; w = int(300*ratio)
    ...
    def attack(self):
        ...
        self.message = f"You hit {self.wild.name} for {dmg}!"; self.cooldown = 0.5
        if self.wild.hp <= 0:
            self.message = f"{self.wild.name} fainted. You found an apricorn!"
            self.player.apricorns += 1; self.active = False
    def throw_ball(self):
        if self.cooldown>0: return
        if self.player.balls <= 0: self.message = "No balls left! Craft with [C]."; return
        self.player.balls -= 1
        ...
        if random.random() < chance:
            ...
            self.player.caught_species.add(self.wild.name); self.active = False
        else: self.message = "Oh no! It broke free."
        self.cooldown = 0.8
...
def main():
    ...
    while running:
            ...
            if e.type == pygame.QUIT: running=False
                ...
                if e.key == pygame.K_ESCAPE:
                    ...
                    if not battle and not victory:
                        for m in world.mons:
                            if player.rect.colliderect(m.rect.inflate(10,10)): battle=Battle(player, m); break
        ...
        if not battle and not victory:
            ...
            cam.center_on(player.rect, view_w, view_h)
        else:
            if battle:
                battle.update(dt)
        ...
        if not victory and len(player.caught_species) >= len(ALL_SPECIES): victory=True

        # draw world to view surface, then scale up to screen
        view_surf.fill(BLACK)
        world.draw(view_surf, cam, view_w, view_h)
        for m in world.mons:
        ...
        # overlays on full screen
        draw_text(screen, f"Zoom:{ZOOM:.2f}  Balls:{player.balls}  Apricorns:{player.apricorns}  Team:{len(player.team)}  Species:{len(player.caught_species)}/{len(ALL_SPECIES)}", (10,8))
        ...
        if victory:
            overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA); overlay.fill((0,0,0,170)); screen.blit(overlay, (0,0))
